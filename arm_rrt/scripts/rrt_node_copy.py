...
#global varaibles
dof = 2
number_of_point_local_planner = 20
upper_limit_distance = 0.4
lower_limit_distance = 0.07
upper_angle_limit_config = 6.2
lower_angle_limit_config = 0.01
max_number_of_node = 350
...
class path_planner:
    def __init__(self):
        self.rate_move=rospy.Rate(10)
        self.obstacles=np.full((900,900),False,dtype=bool)
        self.obstacles[645:820,645:820]=np.full((175,175),True,dtype=bool)
    ...
    def move_arm(self) :
        while (len(self.path)>1):
            for i in range(number_of_point_local_planner):
                next_point=self.list_of_nodes[self.path[-1]].config+(self.list_of_nodes[self.path[-2]].config-self.list_of_nodes[self.path[-1]].config)*i/(number_of_point_local_planner*1.0)
                msg1 = Float64()
                msg1.data= next_point[0]
                msg2 = Float64()
                msg2.data= next_point[1]
                # print("point in path:", self.list_of_nodes[self.path[-1]].config)
                self.pub_joint1.publish(msg1)
                self.pub_joint2.publish(msg2)
                self.rate_move.sleep()
            self.path.pop()
        #publishing final goal pose as this is stored in random_point
        msg1 = Float64()
        msg1.data= self.random_point[0]
        msg2 = Float64()
        msg2.data= self.random_point[1]
        # print("point in path:", self.list_of_nodes[self.path[-1]].config)
        self.pub_joint1.publish(msg1)
        self.pub_joint2.publish(msg2)
    ...
    def local_path_exist(self,point) :
        ...
        for i in range(1,number_of_point_local_planner):           
            new_point_to_investigate=self.random_point + (point-self.random_point)*i/(number_of_point_local_planner*1.0)
    ...
    def near_to_points_with_local_path(self):
        ...
        if(self.distance_with_random(self.list_of_nodes[least_distance_node].config)) :
            if(self.local_path_exist(self.list_of_nodes[least_distance_node].config)) :                
        ...
        while not rospy.is_shutdown():   
            if self.new_point_recieved :
                ...
                if path_exist:                         
                    ...
                    for i in range(50):
                        self.points_sampled_x.append(lower_angle_limit_config +i/50.0*upper_angle_limit_config)
                        self.points_sampled_y.append(lower_angle_limit_config +i/50.0*upper_angle_limit_config)
                    for i in range(50):
                        for j in range(50):
